...
export function mockImageEvent() {
  let fn: JestMockCompat<[string], any>
  ...
  beforeAll(() => {
    fn = vitest.spyOn(
      ...
      'src',
      'set'
    ) as unknown as JestMockCompat<[string], any>
    fn.mockImplementation(function (this: HTMLImageElement, src) {
      const evt = !src || src === IMAGE_FAIL ? 'error' : 'load'
      const event = new window.Event(evt)
